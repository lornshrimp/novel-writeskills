...
- 若小说无分部：使用作品书名。
- 若小说有分部（多部）：必须使用 `readme.md`/`README` 中当前这“一部”的名字作为七猫站内书名，不得误用总书名。
...
1. 章节编号使用“跨卷累加”的**绝对编号**口径，不使用“卷内局部编号”。
2. 计算方式：
   - 读取 `分发记录.md` 中同一书籍ID下全部已记录章节。
   - 依据 `卷号`、`章号`排序后，计算当前章节在全书中的绝对序号。
3. 示例：
   - 第1卷第5章 → 绝对编号第5章。
   - 第2卷第5章 → 绝对编号第95章（假设第1卷共90章）。
4. 若 `分发记录.md` 不存在或无历史记录，则从第1章开始。
5. 页面上若“第X章”不可编辑，则仅用于校验；以平台显示值为准继续流程。
...
- 登录被拦截为验证码登录：标记为“待验证码”，向用户索取验证码后继续；若用户未提供则暂停流程并回报阻塞点。
