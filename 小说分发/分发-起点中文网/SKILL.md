...
- 若小说无分部：使用作品书名。
- 若小说有分部（多部）：必须使用 `README.md` 中当前这“一部”的名字作为起点站内书名，不得误用总书名。
...
2. 确认已获得：账号、密码、起点书籍Id、目标书名、目标卷名。
...
- 登录触发验证码或手机验证：标记为“待验证码”，向用户索取验证码后继续；若用户未提供则暂停流程并回报阻塞点。
