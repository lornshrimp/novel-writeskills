...
1. 在章节标题输入框填写平台合规标题（建议不带 `第X章` 前缀）。
2. 必须确保标题长度不超过 15 字（界面通常显示为 `X/15`）。
3. 若标题超限（如 `27/15`），先压缩标题后再保存稿件。
4. 在正文编辑区粘贴该章正文。
5. 知乎无“作者有话说”输入区域，**不要尝试填写作者有话说**。
...
- 出现二次验证（验证码/滑块）：标记为“待验证”，请用户完成验证后继续。
- 登录后若再次触发二次验证（接口 400/403 或验证弹窗）：仍按“待验证”处理，不得假定登录态永久有效。
...
- 登录后仍可能在后续请求中出现验证或风控（如 captcha 接口异常）；需要保留“待验证”分支，不可当作一次性动作。
- 判定保存成功时建议交叉验证三项：
